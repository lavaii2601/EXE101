...
function AppShell() {
  ...
  const renderScreen = () => {
    if (activeTab === 'overview') return <OverviewScreen onAgentSync={handleAgentSync} syncEvent={syncEvent} onNavigate={setActiveTab} userMode={userMode || 'worker'} subscription={profile?.subscription} userName={profile?.name} />;
