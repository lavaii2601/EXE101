...
class _EmailScreenState extends State<EmailScreen> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(
                '${t('Kết nối Gmail thất bại', 'Failed to connect Gmail')}: $error')));
      }
    } finally {
  ...
  Future<void> _toggleRead(Map<String, dynamic> email) async {
    ...
    try {
      await apiPost(
          wasUnread ? '/email/mark-as-read/$id' : '/email/mark-as-unread/$id');
              ...
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      Text(t('Bộ lọc email', 'Email filters'),
                          style: TextStyle(
                              color: colors.text,
                              fontWeight: FontWeight.w700,
                              fontSize: 17)),
                      IconButton(
                          icon: Icon(AppIcons.emailSearchClear,
                    ...
                    children: [
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(t('Hiện email đã đọc', 'Show read emails'),
                                style: TextStyle(
                                    color: colors.text,
                                    fontWeight: FontWeight.w600,
                                    fontSize: 14)),
                            Text(
                                t('Tắt để chỉ xem email chưa đọc.',
                                    'Turn off to only see unread emails.'),
                                style: TextStyle(
                      ...
                      Switch(
                        ...
                        onChanged: (value) =>
                            setSheetState(() => includeRead = value),
                      ),
                    ],
                  ),
  ...
  void _openShareSheet(Map<String, dynamic> email) {
    final colors = context.read<ThemeController>().colors;
    final t = context.read<LanguageController>().t;
    final businessWorkspaces = context
        .read<WorkspaceController>()
        .workspaces
        .where((w) => w['type'] == 'business')
        .toList();
        ...
        return StatefulBuilder(
          builder: (sheetContext, setSheetState) {
            Future<void> confirmAndShare() async {
              ...
              try {
                await apiPost(
                    '/workspaces/$selectedWorkspaceId/shared-artifacts', {
                  'source_type': 'email_summary',
                  'title': email['subject'] as String? ??
                      t('Email đã chia sẻ', 'Shared email'),
                  'content': {
                ...
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                      content: Text(t('Đã chia sẻ vào không gian doanh nghiệp',
                ...
                if (sheetContext.mounted) {
                  ScaffoldMessenger.of(sheetContext).showSnackBar(SnackBar(
                      content: Text(
                          '${t('Không chia sẻ được', 'Could not share')}: $e')));
                }
              }
            }
            ...
            return Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(
                      t('Chia sẻ vào không gian doanh nghiệp',
                          'Share to a Business workspace'),
                      style: TextStyle(
                          color: colors.text,
                          fontWeight: FontWeight.w700,
                          fontSize: 17)),
                  ...
                  Text(t('Không gian doanh nghiệp', 'Business workspace'),
                      style: TextStyle(
                          color: colors.textMuted,
                          fontWeight: FontWeight.w600,
                          fontSize: 12)),
                  ...
                  Wrap(
                    ...
                    children: businessWorkspaces.map((w) {
                      ...
                      return GestureDetector(
                        onTap: () =>
                            setSheetState(() => selectedWorkspaceId = id),
                        child: Container(
                  ...
                  const SizedBox(height: 18),
                  AppButton(
                      title: t('Xác nhận chia sẻ', 'Confirm sharing'),
                      onPressed: confirmAndShare,
                      loading: sharing),
                ],
              ),
            );
            ...
            Future<void> submit() async {
              ...
              if (to.isEmpty || subject.isEmpty || body.isEmpty) {
                setSheetState(() => error = t(
                    'Vui lòng điền người nhận, tiêu đề và nội dung.',
                    'Please fill in recipient, subject, and body.'));
                return;
              }
              setSheetState(() {
                sending = true;
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      AppField(
                          ...
                          hint: t('Tiêu đề email', 'Email subject')),
                      AppField(
                          label: t('Nội dung', 'Body'),
  ...
  Widget build(BuildContext context) {
    ...
    final canShowBusinessFeatures =
        userMode == 'worker' || userMode == 'business';
    final hasBusinessWorkspace = canShowBusinessFeatures &&
        context
            .watch<WorkspaceController>()
            .workspaces
            .any((w) => w['type'] == 'business');
    ...
    return Scaffold(
      ...
      floatingActionButton: authenticated
          ? FloatingActionButton(
              ...
              child: const Icon(AppIcons.emailCompose, color: Colors.white),
            )
          : null,
      body: SafeArea(
        top: false,
            ...
            if (showSearch) ...[
              ...
              TextField(
                ...
                decoration: InputDecoration(
                  hintText: t('Tìm theo người gửi, tiêu đề...',
                      'Search sender, subject...'),
                  hintStyle: TextStyle(color: colors.inputPlaceholder),
                  prefixIcon: Icon(AppIcons.emailSearch,
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        final s = Map<String, dynamic>.from(
                            suggestions[index] as Map);
                        final title = (s['title'] as String?) ??
                            (s['subject'] as String?) ??
                            t('Lịch hẹn từ email', 'Meeting from email');
                        final meta = s['start_time'] != null
                            ? _formatSuggestionTime(s['start_time'] as String)
                            : ((s['sender'] as String?) ?? '');
                        return Container(
                          width: 210,
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                children: [
                                  Icon(AppIcons.emailMeeting,
                                      size: 16, color: colors.primary),
                                  const SizedBox(width: 6),
                                  Text(t('Lịch hẹn', 'Meeting'),
                                      style: TextStyle(
                                          color: colors.primary,
                                          fontWeight: FontWeight.w700,
                                          fontSize: 11)),
            ...
            if (!authenticated)
                ...
                child: Column(
                  ...
                  children: [
                    AppEmptyState(
                      ...
                      detail: t(
                          'Kết nối Gmail để xem hộp thư và nhận gợi ý thông minh.',
                          'Connect Gmail to view your inbox and get smart suggestions.'),
                    ),
                    const SizedBox(height: 4),
                    AppButton(
              ...
              AppCard(
                child: AppEmptyState(
                  ...
                  detail: searchKeyword.isNotEmpty
                      ? t('Không có kết quả cho "$searchKeyword".',
              ...
              ...emails.map((raw) {
                ...
                final time = _formatEmailTime((email['date'] as String?) ??
                    (email['email_date'] as String?) ??
                    '');
                return Padding(
                  padding: const EdgeInsets.only(bottom: 10),
                  child: InkWell(
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      if (hasBusinessWorkspace)
                                        TextButton.icon(
                                          onPressed: () =>
                                              _openShareSheet(email),
                                          icon: Icon(Icons.share_outlined,
                                              size: 15, color: colors.primary),
                                          label: Text(t('Chia sẻ', 'Share'),
                                              style: TextStyle(
                                                  color: colors.primary,
                                                  fontSize: 11.5,
                                                  fontWeight: FontWeight.w600)),
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child:
            Icon(icon, size: 18, color: active ? Colors.white : colors.primary),
      ),
    );
  }
}
