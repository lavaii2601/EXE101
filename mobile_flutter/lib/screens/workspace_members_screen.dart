...
class _WorkspaceMembersScreenState extends State<WorkspaceMembersScreen> {
  ...
  Future<void> _resolveSeatRequest(String requestId, String action) async {
    ...
    try {
      await apiPost(
          '/workspaces/$workspaceId/seat-requests/$requestId/$action', {});
      _load();
    } catch (_) {}
  }

  String _accessStateLabel(
      String? state, String Function(String, [String?]) t) {
    if (state == 'active') return t('Đang hoạt động', 'Active');
    if (state == 'grace') return t('Sắp hết hạn', 'Expiring soon');
    if (state == 'read_only') {
      return t('Chỉ đọc (đã hết hạn)', 'Read-only (expired)');
    }
  ...
  Future<void> _submitInvite() async {
    ...
    try {
      final data = await apiPost('/workspaces/$workspaceId/invitations',
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(
                '${t('Không gửi được lời mời', 'Could not send invitation')}: $error')));
      }
    } finally {
  ...
  Future<void> _changeRole(String userId, String role) async {
    ...
    try {
      await apiPatch(
          '/workspaces/$workspaceId/members/$userId/role', {'role': role});
      _load();
    } catch (_) {}
  }
  ...
  Future<void> _removeMember(String userId) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(t('Xóa thành viên này khỏi không gian làm việc?',
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(t('Thành viên', 'Members'),
                        style: TextStyle(
                            color: colors.text,
                    ...
                    if (loading)
                      ...
                      if (subscriptionInfo != null) ...[
                        AppCard(
                          child: Column(
                            ...
                            children: [
                              Text(t('GÓI DOANH NGHIỆP', 'BUSINESS PLAN'),
                                  style: TextStyle(
                                ...
                                children: [
                                  ...
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                      color: _accessStateColor(
                                              subscriptionInfo!['access_state']
                                                  as String?,
                                              colors)
                              ...
                              if (subscriptionInfo!['access_state'] ==
                                  'grace') ...[
                                const SizedBox(height: 10),
                                Text(
                                  t(
                      ...
                      AppCard(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(t('DANH SÁCH THÀNH VIÊN', 'MEMBER LIST'),
                                style: TextStyle(
                                    color: colors.primary,
                                    fontWeight: FontWeight.w700,
                                    fontSize: 10,
                      ...
                      if (canManage) ...[
                        ...
                        AppCard(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(t('MỜI THÀNH VIÊN', 'INVITE MEMBER'),
                                  style: TextStyle(
                                      color: colors.primary,
                                      fontWeight: FontWeight.w700,
                                      fontSize: 10,
                              ...
                              Row(
                                children: [
                                  ...
                                  ChoiceChip(
                                    ...
                                    onSelected: (_) =>
                                        setState(() => inviteRole = 'admin'),
                                  ),
                                ],
                              ),
                              const SizedBox(height: 12),
                              AppButton(
                                  title: t('Gửi lời mời', 'Send invitation'),
                                  onPressed: _submitInvite,
                                  loading: inviting),
                              if (inviteResultLink != null) ...[
                                const SizedBox(height: 10),
                                Text(
                                  t('Đã tạo lời mời. Gửi mã này cho người được mời:',
                                      'Invitation created. Send this code to the invitee:'),
                                  style: TextStyle(
                                      color: colors.textMuted, fontSize: 12),
                                ),
                                const SizedBox(height: 4),
                                SelectableText(inviteResultLink!,
                                    style: TextStyle(
                                        color: colors.primary,
                                        fontSize: 12,
                                        fontWeight: FontWeight.w600)),
                                  ...
                                  return Padding(
                                    ...
                                    child: Row(
                                      children: [
                                        ...
                                        TextButton(
                                          onPressed: () => _revokeInvitation(
                                              inv['id'] as String),
                                          child: Text(t('Thu hồi', 'Revoke'),
                                              style: TextStyle(
                                                  color: colors.danger,
                                                  fontSize: 12)),
                                        ),
                        ...
                        if (seatRequests.isNotEmpty) ...[
                          ...
                          AppCard(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(t('YÊU CẦU THÊM CHỖ', 'SEAT REQUESTS'),
                                    style: TextStyle(
                                        color: colors.primary,
                                        fontWeight: FontWeight.w700,
                                        fontSize: 10,
                                  ...
                                  return Padding(
                                    ...
                                    child: Row(
                                      ...
                                      children: [
                                        ...
                                        TextButton(
                                          onPressed: () => _resolveSeatRequest(
                                              r['id'] as String, 'approve'),
                                          child: Text(t('Duyệt', 'Approve'),
                                              style: TextStyle(
                                        ...
                                        TextButton(
                                          onPressed: () => _resolveSeatRequest(
                                              r['id'] as String, 'reject'),
                                          child: Text(t('Từ chối', 'Reject'),
                                              style: TextStyle(
                                                  color: colors.danger,
                                                  fontSize: 12)),
                                        ),
