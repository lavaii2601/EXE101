...
class IntentOrchestrator:
    ...
    def _is_latest_email_summary(self, text):
        has_email = any(term in text for term in ("email", "e-mail", "gmail", "mail", "thu moi", "hop thu"))
        has_latest = any(term in text for term in ("moi nhat", "gan nhat", "vua nhan", "moi nhan", "latest", "newest", "most recent"))
        has_summary = any(term in text for term in ("tom tat", "noi dung", "noi gi", "co gi", "doc ", "summary", "summarize", "what does"))
        return has_email and has_latest and has_summary

    def _latest_email_count(self, text):
        match = re.search(r"\b(\d{1,2})\s*(?:email|e-mail|mail|thu)\b", text)
        if match:
            return max(1, min(int(match.group(1)), 5))
        words = {"mot": 1, "hai": 2, "ba": 3, "bon": 4, "tu": 4, "nam": 5}
        for word, count in words.items():
            if re.search(rf"\b{word}\s+(?:email|e-mail|mail|thu)\b", text):
                return count
        return 1
